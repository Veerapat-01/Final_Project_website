...
function SecurityContent() {
  ...
  useEffect(() => {
    ...
    const darkVars = [
      "--theme-bg-base","--theme-bg-primary","--theme-bg-secondary",
      "--theme-bg-card","--theme-bg-glass","--theme-border",
      "--theme-border-strong","--theme-text-primary","--theme-text-secondary",
      "--theme-text-muted","--theme-accent-green","--theme-accent-red",
      "--theme-accent-light","--theme-accent-glow","--theme-glow-ring-green",
      "--theme-glow-ring-red","--color-background-primary",
      "--color-background-secondary","--color-text-primary",
      "--color-text-secondary","--color-border-tertiary",
    ];
        ...
        <div className={styles.pageInner}>
          ...
          <div className={styles.pageHeader}>
            <div className={styles.headerLeft}>
              ...
              <p className={styles.pageSubtitle}>Manage access, roles and permissions for your team</p>
            </div>
            <button className={styles.addUserBtn} onClick={openAdd} id="add-user-btn">
          ...
          <div className={styles.statsGrid}>
            {[
              { label: "Total Users",   value: stats.total,    sub: "registered accounts", color: "purple" },
              { label: "Active",        value: stats.active,   sub: "currently active",    color: "green"  },
              { label: "Admins",        value: stats.admins,   sub: "with full access",    color: "amber"  },
              { label: "Inactive",      value: stats.inactive, sub: "suspended / idle",    color: "red"    },
            ].map((s) => (
              <div key={s.label} className={cn(styles.statCard, (styles as any)[s.color])}>
              ...
              <div className={styles.panelHeader}>
                <div className={styles.panelTitle}>
                  <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                    ...
                    <path d="M1 14c0-3 2-5 5-5h2c3 0 5 2 5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                  </svg>
                  All Users
              ...
              {filteredUsers.length === 0 ? (
                <div className={styles.emptyState}>
                  <svg width="32" height="32" viewBox="0 0 24 24" fill="none" opacity="0.3">
                    ...
                    <path d="M2 21v-1a8 8 0 0112.596-6.566" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                    <circle cx="19" cy="19" r="3" stroke="currentColor" strokeWidth="1.5" />
                    <path d="M19 17v2.5M19 21v.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                  </svg>
                        ...
                        <tr key={u.id} className={styles.fadeIn}>
                          <td>
                            <div className={styles.userCell}>
                              <div className={cn(styles.avatar, getAvatarColor(u.id))}>
                                {getInitials(u.name)}
                              </div>
                              <div>
                          ...
                          <td>
                            <span className={cn(styles.roleBadge, ROLE_BADGE[u.role])}>
                              {u.role}
                            </span>
                          </td>
                          <td>
                            <span className={cn(styles.statusBadge, u.status === "Active" ? styles.statusActive : styles.statusInactive)}>
                          ...
                          <td>
                            <div className={styles.actionBtns} style={{ justifyContent: "flex-end" }}>
                              ...
                              >
                                <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                                  <path d="M3 5h10M6 5V3h4v2M13 5l-1 8H4L3 5" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                                </svg>
                              </button>
                            </div>
                          </td>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              ...
              <div className={styles.panel}>
                <div className={styles.panelHeader}>
                  <div className={styles.panelTitle}>
                    <svg width="13" height="13" viewBox="0 0 16 16" fill="none">
                      <path d="M8 1l2 4h4l-3.2 2.4 1.2 4L8 9 4 11.4l1.2-4L2 5h4z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round" />
                    </svg>
                    Roles & Permissions
                  </div>
                </div>
                {(["Admin", "Operator", "Viewer", "Auditor"] as Role[]).map((role) => (
                  <div key={role} className={styles.roleCard}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div className={styles.roleCardDesc}>{ROLE_META[role].desc}</div>
                    </div>
                    <div className={styles.roleCardCount}>{roleCounts[role]}</div>
                  </div>
                ))}
              </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setShowAddModal(false)}>
          <div className={styles.modalCard} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              {/* Error */}
              {formError && (
                <div style={{ color: "#f87171", fontSize: "12px", fontFamily: "'DM Mono', monospace" }}>
                  ⚠ {formError}
                </div>
              )}
            </div>
        ...
        <div className={styles.modalOverlay} onClick={() => setDeletingUser(null)}>
          <div className={styles.modalCard} onClick={(e) => e.stopPropagation()} style={{ maxWidth: 400 }}>
            <div className={styles.modalHeader}>
              ...
              <button className={styles.modalClose} onClick={() => setDeletingUser(null)}>✕</button>
            </div>
            <div className={styles.deleteModalBody}>
              <div className={styles.deleteIcon}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  ...
                  <path d="M10 11v6M14 11v6" stroke="#f87171" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              </div>
            ...
            <div className={styles.deleteConfirmFooter}>
              <button className={styles.cancelBtn} onClick={() => setDeletingUser(null)}>Cancel</button>
              <button className={styles.deleteConfirmBtn} onClick={handleDelete}>Delete User</button>
            </div>
          </div>
        </div>
      )}
...
export default function SecurityPage() {
  return (
    <Suspense fallback={<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', background: '#f5f6fa' }}>Loading security...</div>}>
      <SecurityContent />
    </Suspense>
  );
}
